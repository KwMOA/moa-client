...
int Network::connectWithServer(int serverModule, const char* ip, unsigned short port)
{
    ...
    int status = fcntl(fdArray[i].fd, F_SETFL, fcntl(fdArray[i].fd, F_GETFL, 0) | O_NONBLOCK);
...
int Network::receiveData(int serverModule)
{
    ConnectInfo* connectInfo = getConnectInfo(serverModule);
...
bool Network::sendPacket(int serverModule, const char* data, int dataSize)
{
    ...
    if(sendData(connectInfo->fd, packetArray, sizeof(dataHeader) + dataSize) == false)
    {
        return false;
    }
...
bool Network::disconnectWithServer(int serverModule)
{
    ConnectInfo* connectInfo = getConnectInfo(serverModule);
