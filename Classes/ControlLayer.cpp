...
bool ControlLayer::init()
{
	...
	for (buttonCount = 0; buttonCount < 12; buttonCount++) {
...
void ControlLayer::DetailUnitCallback(cocos2d::Ref * pSender, cocos2d::ui::Widget::TouchEventType eventType)
{
	...
	ButtonInfo* diva = gameScene->getObjectInfoList()->getInfoByName(clickedName);
...
void ControlLayer::makeBuildingButton(string nomal, string select, string disable, int buttonCount) {
		...
		button = Button::create(nomal, select, disable);
		button->retain();
		...
		csBtn->setState(BUTTON_STATE_NONE);
