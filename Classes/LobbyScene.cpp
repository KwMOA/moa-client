...
//    addChild(menuButtonLayer, 0, TAG_MENU_BUTTON_LAYER);
    ...
    lobbyMainLayer->retain();
    
    lobbyMainLayer->setAnchorPoint(Vec2(0, 0));
    lobbyMainLayer->setPosition(Vec2(0, 0));
    
    lobbyGameLayer = LobbyGameLayer::create();
    lobbyGameLayer->retain();
    
    lobbyGameLayer->setAnchorPoint(Vec2(0, 0));
    lobbyGameLayer->setPosition(Vec2(0, 0));
    
    lobbyAIGameLayer = LobbyAIGameLayer::create();
    lobbyAIGameLayer->retain();
    
    lobbyAIGameLayer->setAnchorPoint(Vec2(0, 0));
    lobbyAIGameLayer->setPosition(Vec2(0, 0));
...
//    lobbyBoardLayer->setPosition(Vec2(0, 0));
    ...
    pushLayer(lobbyMainLayer);
    
    currentLayer = CURRENT_LAYER_MAIN;
...
//
//    switch (currentLayer) {
//        case CURRENT_LAYER_MAIN:
//        {
//            removeChildByTag(TAG_LOBBY_MAIN_LAYER, false);
//            break;
//        }
...
//    }
    ...
    switch (layerNumber) {
        ...
        case CURRENT_LAYER_GAME:
        {
            pushLayer(lobbyGameLayer);
            break;
        }
        case CURRENT_LAYER_AI_GAME:
        {
            pushLayer(lobbyAIGameLayer);
            break;
        }
        case CURRENT_LAYER_RANK_GAME:
        {
            pushLayer(lobbyRankGameLayer);
            break;
        }
        case CURRENT_LAYER_CHANNEL:
        {
            pushLayer(lobbyChannelLayer);
            break;
        }
        case CURRENT_LAYER_FRIEND:
        {
            pushLayer(lobbyFriendLayer);
            break;
        }
        case CURRENT_LAYER_CLAN:
        {
            pushLayer(lobbyClanLayer);
            break;
        }
        case CURRENT_LAYER_RANKING:
        {
            pushLayer(lobbyRankingLayer);
            break;
        }
        case CURRENT_LAYER_BOARD:
        {
            pushLayer(lobbyBoardLayer);
            break;
        }
